...
- ✅ **NEW: hidden gems got their first 168-destination audit**, exposing that the sentiment floor — not the POI pool — was the real bottleneck: destinations returning a gem went 44% → 54%. A committed, resumable `audit_gems.py` and a lexicon calibration script are now permanent entry points for any future gem work.
- 🔴 **FIXED: gem name matching derived demonyms as POI names** ("Egyptian Museum" → bare token `egyptian`, matching unrelated comments). The guard now checks whether a derived core is an ordinary English word against a generated word list, rather than only a length threshold.
- ✅ **NEW: YouTube video narration (transcripts + descriptions) is a second price-grounding source**, alongside comments — people quote costs out loud in videos far more than in comments (Jaipur: 24 money-shaped chunks vs. 0). Costs almost nothing extra since video IDs are already stored from the comment backfill.
- 🔴 **FIXED: no user-typed field was bounded.** A "monkey testing" pass found unbounded destination strings, hop counts and date ranges could exhaust memory or silently mis-plan a trip. New `core/validation.py` centralises caps, normalisation and `Literal` field types (`pace`, `scope`, `crowd_preference`, `destination_mode`) mirrored by the frontend's `apps/web/lib/limits.ts`.
- 🔴 **FIXED: voice mode had never actually spoken in production.** A stale-closure bug and a conflated "voice mode on" / "mic listening" flag meant `speak()` was never called on a reply. Voice moved into pure `apps/web/lib/voice.ts` + `apps/web/hooks/useVoice.ts`, gained 78 new frontend tests, and now supports Hindi speech recognition/synthesis alongside English.
- ✅ **NEW: `generate_itinerary()` is measured for the first time** — `core/timing.py` records per-stage wall-clock, revealing the real cold-start cost is a one-time model load (~18.6s after every Railway redeploy) and that the LLM retry backoff (75–225s) can exceed the request timeout, making the degraded-itinerary fallback unreachable under sustained transient errors.
- ✅ **NEW: voice-mic UI redesigned and a full E2E accessibility pass shipped** across every surface (landing, auth, wizard, dashboard, itinerary, account, admin, nav) in response to live user reports (issues #30/#31), with zero backend regression.
- Suite green throughout (883+ backend tests, 122+ frontend tests); ruff, mypy and `tsc --noEmit` clean. Full detail per release: `TECHNICAL_DOCUMENTATION.md` §14 (v10.39.0–v10.48.0), `docs/system-design.md` §16, `docs/UI_UX_AUDIT_2026-07-29.md`, `docs/NEXT_SESSION_TODO.md`.
