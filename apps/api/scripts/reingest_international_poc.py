"""International POC-magnet cities re-ingestion batch (NEXT_SESSION_TODO.md
item 2, continuation of scripts/reingest_india_tier23.py).

A 2026-07-23 live read-only audit of all 134 non-India KNOWN_DESTINATIONS
found 111 failing the data-completeness gate (mostly wiki=0, some also
food/restaurant-category-dominated on OSM). That's too large a batch to
re-ingest in one run without high Overpass/Wikivoyage rate-limit risk, so
this script targets just the highest-value first slice: the top POC-magnet
international cities explicitly named in NEXT_SESSION_TODO.md's item 2 —
Tokyo, Rome, Barcelona, Amsterdam, Berlin, Dubai, Singapore, Kyoto, Vienna.
~100+ destinations remain in the broader backlog after this batch (see the
full failure list captured in this run's JSON summary's sibling audit, or
re-run eval/run_data_completeness_check.py-style scroll per destination).

Same pattern as reingest_india_tier23.py: re-ingests both OSM POIs and
Wikivoyage text via ingest_osm_pois()/ingest_wikivoyage() (retry/backoff +
delete-then-upsert stale-point cleanup + the 2026-07-23 adaptive radius-
expansion fallback are all already built in), upserts a
destination_ingestion_state row, and writes a JSON summary to scripts/out/.
...
logger = logging.getLogger("reingest_international_poc")
...
DESTINATIONS = [
    "Tokyo", "Rome", "Barcelona", "Amsterdam", "Berlin", "Dubai",
    "Singapore", "Kyoto", "Vienna",
]
...
async def main() -> None:
    ...
    out_path = f"scripts/out/reingest_international_poc_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
