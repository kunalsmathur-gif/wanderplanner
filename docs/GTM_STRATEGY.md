...
| 4 | Refinement-fidelity eval suite; publish vs-ChatGPT results | Builds on `docs/eval-set.csv` | 🟡 Measured (v10.18.2, 2026-07-13) — live WanderPlanner fidelity 0.771 / honesty 4-4; ChatGPT recall 1.00 but 0.75 unverifiable + 0/4 honesty (invented "Wizarding World Goa"); Claude Sonnet recall 0.98, 0.79 unverifiable, verbally honest on impossibles. Wedge = trust + follow-through, not recall. **Publish blocked on fixing 3 zero-pin recall bugs (Kyoto/Goa/Bengaluru) + Barcelona generation-compliance, then a repeat live run** — see NEXT_SESSION_TODO |
