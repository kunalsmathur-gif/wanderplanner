...
| 4 | Refinement-fidelity eval suite; publish vs-ChatGPT results | Builds on `docs/eval-set.csv` | 🟢 Gate passed on numbers (v10.19.0, 2026-07-13) — recall bugs fixed (detection prompt + themes backstop + exact-beats-fuzzy verification) and pin inclusion now enforced structurally. Live: fidelity **0.904**, recall 0.854, inclusion/stability 0.938 (1.00 on every pinned case), honesty 4/4 — vs ChatGPT (recall 1.00, **0.74 unverifiable**, 0/4 honesty) and Claude Sonnet (0.98, 0.79 unverifiable, verbally-honest nuance). **Remaining before publish:** rerun to clear RF-010 (transient Gemini 503 zeroed one case) and optionally chase RF-012 Mumbai recall; then write + publish the comparison piece with the Claude verbal-honesty disclosure |
