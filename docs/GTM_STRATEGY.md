...
- **Best mode of contact: WhatsApp**, not a form-then-email flow — `wa.me/<agent_number>?text=<prefilled itinerary summary>` (destination, dates, pax, budget tier, shareable itinerary link). Matches where Indian users and offline agents already operate; a generic web form loses this audience. **Response commitment (now live):** the CTA persists the lead via `POST /api/agent-leads`, sends an immediate Resend confirmation email with an explicit 24-hour SLA, and then opens the WhatsApp deep link. Hourly SLA checks escalate unanswered leads at 24h and reassure the user at 48h. This is deliberately email-only infrastructure at this stage (no phone/OTP sign-in) — see `docs/PRD.md` Clarification #19 for why phone-based auth was evaluated and deferred.
...
- **Metrics now on the admin dashboard** (issue #63): `agent_leads.created_total`, `responded_total`, `escalated_total`, `reassurance_sent_total`, `response_time_avg_hours`, `response_time_p50_hours`, `response_time_p90_hours`, `sla_breach_rate`, `marked_booked_total`, plus `top_destinations`. These are the concrete numbers that should gate "is it time to bring on a second person handling leads," not a vague sense of being busy.
