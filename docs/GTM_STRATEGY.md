...
- **Best mode of contact: WhatsApp**, not a form-then-email flow — `wa.me/<agent_number>?text=<prefilled itinerary summary>` (destination, dates, pax, budget tier, shareable itinerary link). Matches where Indian users and offline agents already operate; a generic web form loses this audience. **Response commitment (⏳ planned, issue #62):** whichever channel a lead lands on, the founder made an explicit 24-hour response SLA to faculty during Deploy review — enforced via an immediate Resend confirmation email + an hourly escalation job, with a 48h auto-reassurance email to the user if still unanswered, so a pilot lead is never left in silence even if the sole founder is personally unreachable. This is deliberately email-only infrastructure at this stage (no phone/OTP sign-in) — see `docs/PRD.md` Clarification #19 for why phone-based auth was evaluated and deferred.
...
- **Metrics to add to the existing admin dashboard** (⏳ planned, issue #63; same pattern as Gemini/Pexels usage tracking): CTA click-through rate, lead → agent-response rate, lead → booked-trip conversion, **plus the SLA-health numbers that gate whether the solo-founder response model is still working** — average/p50/p90 response time and an `agent_lead_sla_breach_rate` (share of leads that hit the 24h escalation). These are the concrete numbers that should gate "is it time to bring on a second person handling leads," not a vague sense of being busy.
