...
### Bet 4 — A self-reinforcing data flywheel ⭐ NEW (issue #32, shipped v10.74)

Every generic-LLM-wrapper competitor answers from parametric memory alone — no mechanism to get better with usage short of the underlying model vendor's own retraining, which they don't control. WanderPlanner now does: every successful live generation is stored in a new `generated_itineraries` Qdrant collection and retrieved as additional few-shot grounding for future similar requests, alongside the scraped `itinerary_corpus`. This is a genuine moat property, not a vanity metric — usage itself compounds the product's own reference material, with zero incremental scraping or paid-data cost. Early and still coarse (quality weighting today is a simple heuristic, not the richer engagement-signal model in `docs/rag-strategy.md` §10), but the mechanism is live in production, not roadmap. Also closes a retention gap found in live testing: signed-in users can now resume their most recently generated itinerary (`user_last_itinerary`, issue #65) from the Account page or by asking Anya, instead of losing it the moment client-side session state clears.
