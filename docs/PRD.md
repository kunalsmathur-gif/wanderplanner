...
| Question | Decision |
|---|---|
| The Deploy-section feedback plan describes flagging "this itinerary missed the mark" or reacting to a specific day/place, tied to the exact request that produced it — is this built? | **Yes (issue #64).** `itinerary_feedback` table + `POST/PATCH /api/itinerary-feedback`, an itinerary-level "missed the mark" flag (`ItineraryFeedbackFlag.tsx`) and a day/place-level thumbs-up/down in `ItineraryTimeline.tsx`, plus admin-visible feedback volume and negative-feedback rate by destination (`GET /api/admin/metrics/summary`). This is a **consumer-side-only** commitment; the agent/B2B side of feedback stays deliberately manual (hand-onboard a small number of real agents for free, talk to them directly, automate only once a few are willing to pay) — see the original Deploy-section answer for the reasoning. See `docs/system-design.md` §9C. |
| What does "tied to the exact request that produced it" mean concretely? | Every feedback row stores the originating `TripConfig`/generation request (destination, dates, budget, pace, themes, pinned POIs) as a `trip_config_snapshot`, not just a bare itinerary ID — so a pattern (e.g. "Bali keeps getting flagged for touristy output") is queryable later, not just anecdotal. Verified: the snapshot is copied at submit time and does not change if the live trip config is later edited/regenerated. Two granularities: an itinerary-level "missed the mark" flag, and a day/place-level thumbs-up/down in `ItineraryTimeline.tsx` (changeable via a `PATCH` that flips sentiment on the same row rather than creating a duplicate). |
