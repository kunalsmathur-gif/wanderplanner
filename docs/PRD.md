...
> **Further implementation status (v10.32) — ⚠️ commercial-licensing fix + new Airbnb-based stay estimates:** the `_COST_MATRIX` recalibration that followed v10.28 (via budgetyourtrip.com for `stay_per_night_pp`, via Numbeo for premium-tier `food_per_day_pp`) turned out to have a real problem — **both sources' Terms of Service prohibit the kind of commercial use this app makes of the data** without a paid license. This session re-sourced `stay_per_night_pp` onto compliant data: real Wikivoyage (CC BY-SA 3.0) per-listing hotel prices reconstructed to the same dollar figures via an empirically-derived multiplier (documented in `_COST_MATRIX`'s docstring). Also added a genuinely new user-facing capability: when a user explicitly asks for an Airbnb/vacation-rental stay, or when a destination has no usable Wikivoyage hotel pricing at all (e.g. Istanbul), the estimator now grounds the stay figure in real Inside Airbnb (CC BY 4.0) data instead of the flat table. **⚠️ Still open, top priority for next session:** the Numbeo-sourced premium-tier `food_per_day_pp` figures have the identical licensing problem and are **not yet remediated** — do not treat them as a settled, compliant fact until fixed. See `TECHNICAL_DOCUMENTATION.md` §14 v10.32 and `docs/NEXT_SESSION_TODO.md`'s top section for full detail.
