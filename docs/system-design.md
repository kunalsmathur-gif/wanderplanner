...
**Follow-up correction (2026-08-31):** Nearby Search (New) has no "Essentials" tier — only **Pro** (`id`/`displayName`/`location`/`types`, $9.60/1,000 calls in India, 35,000/month free) and **Enterprise** (adds `rating`/`userRatingCount`/hours/phone/price level/website, $10.50/1,000, only 7,000/month free), billed at the *entire request's* highest-tier field. The original field mask requested `rating`/`userRatingCount`, so every call was already billing at Enterprise rates. `FIELD_MASK` was trimmed to Pro-only fields and `google_places_cost_per_1000_calls_usd` corrected from a guessed `32.0` to the verified `9.60` — the larger free-call cap matters more than the small per-call price gap at this scale. Ratings can be re-added later if `poi_provider_usage` data justifies Enterprise.
